...
const int nptbin = 10;
std::string ptbin[nptbin] = { "nosel","pt20t30","pt30t50","pt50t70",
   "pt70t100","pt100t140","pt140t200","pt200t300","pt300t670","pt670t1000" };
...
const int nbtag = 3;
std::string btag[nbtag] = {"_nosel","_CSVM","_UNTAGCSVM"};
//const int nbtag = 3;
//std::string btag[nbtag] = {"_nosel","_combSvx0p244","_UNTAGbc"};
...
void doSys(TH1D *hINIT,TH1D *hRW,TH1D *hRW2,TH1D *hDOWN,TH1D *hUP)
{
   ...
   for(int iy=0;iy<=nby+1;iy++)
	...
	if( useCorMC )
	  {			    
	     ...
	     hDOWN->SetBinContent(iy,v_down);
	     hDOWN->SetBinError(iy,v_mc_rw_err);
	     ...
	     if( doErrSym )
		  ...
		  if( onlyRW )
		  ...
		  else
		    {
		       ...
		       v_down = v_mc-delta2;
		       if( deltaAbs2 > v_mc && delta2 > 0 ) v_down = 0.;
		       if( deltaAbs2 > v_mc && delta2 <= 0 ) v_down = 2*v_mc;		       
		    }		  
	       }	     
	     hDOWN->SetBinContent(iy,v_down);
	     hDOWN->SetBinError(iy,v_mc_err);
	     hUP->SetBinContent(iy,v_up);
	  }		       
     }   
}

void plotSF(TH1D *hRW,TH1D *hNORW,TH1D *hDOWN,TH1D *hUP,TH1D *hDATA,std::string nname)
{
...
//   c1_1->SetLogy(1);
   ...
   ratioErr(c1,hRW,hNORW,hDOWN,hUP,rw);
...
void plotSYS(TH1D *hRW,TH1D *hRW2,TH1D *hNORW,TH1D *hDOWN,TH1D *hUP,TH1D *hDATA,std::string nname)
{   
...
void valSys(TH1D *hNOM,TH1D *hDOWN,TH1D *hUP)
{   
   for(int b=0;b<=hNOM->GetXaxis()->GetNbins()+1;b++)
     {
	double vnom = hNOM->GetBinContent(b);
	double vdown = hDOWN->GetBinContent(b);
	double vup = hUP->GetBinContent(b);
	if( vnom < vdown ) 
	  {
	     if( fabs(vnom-vdown) < 10E-6 )
	       {
		  vdown = vnom;
		  hDOWN->SetBinContent(b,vdown);
	       }
/*FIXME	     else
	       {				 
		  std::cout << "Validation failed" << std::endl;
		  std::cout << b << " vnom=" << vnom << " vdown=" << vdown << std::endl;
		  exit(1);
	       }*/
	  }
     }
