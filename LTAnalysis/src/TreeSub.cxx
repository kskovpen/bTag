#include "../include/TreeSub.h"

LTANA::TreeSub::TreeSub(TChain *ch,char *fname,std::string treename)
{
...
LTANA::TreeSub::~TreeSub()
{
   if (!fChain) return;
   delete fChain->GetCurrentFile();
}

Int_t LTANA::TreeSub::GetEntry(Long64_t entry)
{
   if (!fChain) return 0;
   return fChain->GetEntry(entry);
}

Long64_t LTANA::TreeSub::LoadTree(Long64_t entry)
{
   if (!fChain) return -5;
...
void LTANA::TreeSub::registerInputBranches(TChain *tr)
{
...
void LTANA::TreeSub::Init(TChain *tree)
{
   if (!tree) return;
   fChain = tree;
   fCurrent = -1;
