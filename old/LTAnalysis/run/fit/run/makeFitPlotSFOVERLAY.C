void makeFitPlotSFOVERLAY()
{
   ...
   const int nf = 4;
   std::string fv[nf] = {"Sd0t1","Sd0t2","Sd0t3","Sd0mu"};
   ...
   TLegend *leg = new TLegend(0.78,0.90,0.90,0.55);
   ...
   for(int f=0;f<nf;f++)
     {
	std::string fname = "results/sfoverlay/fit_EFF_beauty_"+fv[f]+"_"+sel+".root";
	...
	if( f == 1 ) {h->SetMarkerStyle(21);h->SetMarkerColor(kBlack);h->SetLineColor(kBlack);}
	...
	std::string tit = "";
	if( f == 0 ) tit = "S_{d_{0}}(t_{1})";
	if( f == 1 ) tit = "S_{d_{0}}(t_{2})";
...
*/
   ...
   std::string fsave = "pics/fit_SFOVERLAY_"+sel+".eps";
